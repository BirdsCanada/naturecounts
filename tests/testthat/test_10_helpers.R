...
test_that("format_zero_fill() by", {
  ...
  expect_message(rc1_filled <- format_zero_fill(rc1,
    ...
    suppressMessages() 
  expect_named(rc1_filled, c("collection", "Locality", "species_id", 
                             "ObservationCount"))
  expect_true(nrow(rc1_filled) > 0)
})
...
test_that("format_zero_fill() different 'fill' columns", {
  rc1 <- dplyr::filter(
    test_rc, .data$CommonName %in% c("Monarch",
                                     "Black Swallowtail",
                                     "Red Admiral"),
    .data$AllSpeciesReported == "Yes") %>%
...
test_that("format_zero_fill() extra events columns", {
  ...
  expect_error(format_zero_fill(test_rc, extra_event = "test", verbose = FALSE),
...
test_that("format_zero_fill() with SQLite database", {
  ...
  DBI::dbExecute(hofi_sql, "DELETE FROM naturecounts WHERE AllSpeciesReported == 'No'")
