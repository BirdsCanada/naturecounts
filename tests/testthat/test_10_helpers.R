...
test_that("format_dates() with data frame", {
  for (i in 1:2) {
    ...
    expect_s3_class(f$date, "Date")
    expect_type(f$doy, "double")
    expect_equal(
      lubridate::as_date(paste(f$survey_year, f$survey_month, f$survey_day)),
      f$date
    )
...
test_that("format_dates() with SQLite database", {
  ...
  file.copy(system.file("extdata", "bcch.nc", package = "naturecounts"), ".")
  file.copy(system.file("extdata", "hofi.nc", package = "naturecounts"), ".")

  for (x in 1:2) {
    if (x == 1) {
      i <- DBI::dbConnect(RSQLite::SQLite(), "bcch.nc")
    } else if (x == 2) {
      i <- DBI::dbConnect(RSQLite::SQLite(), "hofi.nc")
    }
    expect_silent(f <- format_dates(i)) %>%
      expect_s4_class("SQLiteConnection")
    expect_silent(
      f <- dplyr::tbl(f, "naturecounts") %>%
        dplyr::collect()
    )
    expect_true(all(c("doy", "date") %in% names(f)))
    expect_type(f$date, "character")
    expect_type(f$doy, "integer")
    expect_equal(
      lubridate::as_date(paste(f$survey_year, f$survey_month, f$survey_day)),
...
test_that("format_dates() overwrite", {
  ...
  # SQL
  s <- DBI::dbConnect(RSQLite::SQLite(), "bcch.nc")
...
test_that("format_zero_fill() checks for columns", {
  ...
  t1 <- data.frame(species_id = 1, ObservationCount = 1)
...
test_that("format_zero_fill() detects NA Sampling Events", {
  ...
  rc1$SamplingEventIdentifier[
    rc1$SamplingEventIdentifier == "RCBIOTABASE-5553-1"
  ] <- NA
  expect_message(
    b <- format_zero_fill(rc1),
    "There are missing values in 'by'"
  ) %>%
    expect_message("Converted 'fill'")
...
test_that("format_zero_fill() adds zeros", {
  # No zeros to add
  t1 <- data.frame(
    SamplingEventIdentifier = c(1:10),
    AllSpeciesReported = "Yes",
    species_id = 1,
    ObservationCount = "1"
  )
  ...
  # Add zeros
  t1 <- data.frame(
    SamplingEventIdentifier = c(1, 1, 1, 2, 2, 2, 1, 1, 1, 2, 2, 2),
    AllSpeciesReported = "Yes",
  ...
  texp <- dplyr::select(t1, -"AllSpeciesReported") %>%
    rbind(data.frame(
      SamplingEventIdentifier = c(1, 2),
      species_id = c(3, 1),
      ObservationCount = 0
    )) %>%
    dplyr::arrange(.data$SamplingEventIdentifier, .data$species_id)
  expect_message(t <- format_zero_fill(t1), "multiple observations")
  expect_equal(t, texp)
})
...
test_that("format_zero_fill() adds zeros in real example", {
  ...
  expect_silent(
    rc1_fill <- format_zero_fill(rc1, species = 19360, verbose = FALSE)
  ) %>%
    expect_s3_class("data.frame")
  expect_equal(unique(rc1_fill$species_id), 19360)
  expect_equal(nrow(rc1_fill), length(unique(rc1$SamplingEventIdentifier)))
...
test_that("format_zero_fill() by", {
  ...
  # Custom
  expect_silent(
    rc1_filled <- format_zero_fill(
      rc1,
      verbose = FALSE,
      by = c("collection", "date", "Locality", "DurationInHours")
    )
  ) %>%
    expect_named(c(
      "collection",
  ...
  # Custom warning
  expect_message(
    rc1_filled <- format_zero_fill(rc1, by = c("collection", "Locality")),
    "multiple observations"
  ) %>%
    suppressMessages()
  expect_named(
    rc1_filled,
    c("collection", "Locality", "species_id", "ObservationCount")
  )
  expect_true(nrow(rc1_filled) > 0)
})
...
test_that("format_zero_fill() extra species columns", {
  ...
  expect_error(
    format_zero_fill(test_rc, extra_species = "species"),
    "Some 'extra_species' are not in the data"
  )
  expect_message(b <- format_zero_fill(test_rc, extra_species = "species_id"))
  expect_equal(b, b1)
  expect_message(
    format_zero_fill(test_rc, extra_species = "record_id"),
...
test_that("format_zero_fill() extra events columns", {
  ...
  expect_error(
    format_zero_fill(test_rc, extra_event = "test", verbose = FALSE),
    "Some 'extra_event' are not in the data"
  )

  # Ignores non unique columns
  expect_message(
    format_zero_fill(test_rc, extra_event = "record_id"),
...
test_that("format_zero_fill() with SQLite database", {
  ...
  bcch_sql <- DBI::dbConnect(RSQLite::SQLite(), "bcch.nc")
  hofi_sql <- DBI::dbConnect(RSQLite::SQLite(), "hofi.nc")
  DBI::dbExecute(
    hofi_sql,
    "DELETE FROM naturecounts WHERE AllSpeciesReported == 'No'"
  )

  # No zeros to add
  expect_message(
    b <- format_zero_fill(bcch_sql),
  ...
  expect_equal(
    ...
    dplyr::select(bcch, names(b)) %>%
      ...
      dplyr::arrange(SamplingEventIdentifier)
  )
...
test_that("format_zero_fill() checks for size", {
  temp <- expand.grid(species_id = 1:1001, SamplingEventIdentifier = 1:5001) %>%
