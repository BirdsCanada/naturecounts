...
test_that("Code checks work on collections", {
  ...
  expect_silent(collections_check(c(
    "ABATLAS1",
    "ABBIRDRECS",
    "RCBIOTABASE"
  ))) %>%
  ...
  expect_error(
    collections_check("my_favourite_collection"),
    "'collections' must be either NULL \\(return all collections\\)"
  )
  expect_error(
    collections_check(1042),
    "'collections' must be either NULL \\(return all collections\\)"
  )
})
...
test_that("Code checks work on project_ids", {
  ...
  expect_error(
    projects_check("my_favourite_collection"),
    "'project_ids' must be either NULL or a vector of valid"
  )
  expect_error(
    projects_check("ABATLAS1"),
    "'project_ids' must be either NULL or a vector of valid"
  )
})
...
test_that("iba_check correct", {
  for (s in c("AB001", "ab001", "YK007")) {
    expect_silent(iba_check(s))
  }
  for (s in c("A001", "AB999", "YK000")) {
    expect_error(iba_check(s))
  }
})
...
test_that("bcr_check correct", {
  for (s in c("1", 10, 67)) {
    expect_silent(bcr_check(s))
  }
  for (s in c("bcr.1", "99", 100)) {
    expect_error(bcr_check(s))
  }
})

# Site Type ---------------------------------------------------------------
test_that("site_type_check correct", {
  for (s in c("IBA", "iba", "iBA")) {
    expect_silent(site_type_check(s))
  }
  for (s in c("I", NA, "AB001")) {
    expect_error(site_type_check(s))
  }
})
...
test_that("filter_redundancy correct", {
  ...
  f <- list(
    collections = "RCBIOTABASE",
    species = 14280,
    region = list(country = "CA", statprov = "ON", subnational2 = "CA.MB.07")
  )
  f2 <- f
  ...
  f <- list(
    collections = "RCBIOTABASE",
    species = 14280,
    fields_set = "BMDE2.00-min",
    fields = "CollectionYear"
  )
  f2 <- f
  f2['fields'] <- list(NULL)
