...
test_that("Search for multiple matches", {
  expect_silent(c <- region_search(c("Ontario", "British Columbia"),
                                     type = "statprov"))
  expect_equal(nrow(c), 2)

  expect_silent(c <- region_search(c("Colombia", "Canada"),
                                     type = "country"))
  expect_equal(nrow(c), 2)
})
...
test_that("Search for region codes", {
  ...
  expect_silent(region_search(type = "country")) %>%
    expect_equal(meta_country_codes())

  expect_silent(region_search(type = "statprov")) %>%
