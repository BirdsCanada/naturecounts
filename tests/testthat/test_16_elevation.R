...
test_that("elevation_download() hits API with all expected inputs.", {
  expect_silent(elev_sf_pt <<- suppressMessages(elevation_download(data_fmt(bcch,
                                                                      coord_lon = "longitude",
                                                                      coord_lat = "latitude",
                                                                      crs = 4326),
                                                             dl_path = "./testdir")))
  expect_silent(elev_sf_poly <<- suppressMessages(elevation_download(data_buff(data_fmt(bcch,
                                                                                  coord_lon = "longitude",
                                                                                  coord_lat = "latitude",
                                                                                  crs = 4326)),
                                                               dl_path = "./testdir")))
  expect_silent(elev_terra_pt <<- suppressMessages(elevation_download(data_fmt(terra::vect(bcch,
                                                                                     crs = "epsg:4326",
                                                                                     geom = c("longitude", "latitude"))),
                                                                dl_path = "./testdir")))
  expect_silent(elev_terra_poly <<- suppressMessages(elevation_download(data_buff(data_fmt(terra::vect(bcch,
                                                                                                 crs = "epsg:4326",
                                                                                                 geom = c("longitude", "latitude")))),
                                                                  dl_path = "./testdir")))
})
...
test_that("Results downloaded from elevation_download() have expected features.", {
  ...
  expect_true(all(file.size(list.files("./testdir/TerrainTiles", full.names = TRUE)) > 100000)) # Check files contain a reasonable amount of data
