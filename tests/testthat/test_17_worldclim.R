...
test_that("Results downloaded from elevation_download() have expected features.", {
  ...
  expect_true(terra::is.related(tavg_sf_pt,
                                terra::project(terra::vect(suppressMessages(data_fmt(bcch,
                                                                      coord_lon = "longitude",
                                                                      coord_lat = "latitude",
                                                                      crs = 4326))), terra::crs(tavg_sf_pt)),
                                "contains"))
  expect_true(terra::is.related(tavg_sf_poly,
                                terra::project(terra::vect(suppressMessages(data_buff(data_fmt(bcch,
                                                                                coord_lon = "longitude",
                                                                                coord_lat = "latitude",
                                                                                crs = 4326)))), terra::crs(tavg_sf_poly)),
                                "contains"))
  expect_true(terra::is.related(tavg_terra_pt,
                                terra::project(suppressMessages(data_fmt(terra::vect(bcch,
                                                                      crs = "epsg:4326",
                                                                      geom = c("longitude", "latitude")))), terra::crs(tavg_terra_pt)),
                                "contains"))
  expect_true(terra::is.related(tavg_terra_poly,
                                terra::project(suppressMessages(data_buff(data_fmt(terra::vect(bcch,
                                                                                crs = "epsg:4326",
                                                                                geom = c("longitude", "latitude"))))), terra::crs(tavg_terra_poly)),
                                "contains"))
})
...
test_that("worldclim_extract() throws appropriate error when inappropriate file provided to worldclim_data or argument is missing.", {
  expect_error(worldclim_extract(suppressWarnings(suppressMessages(data_fmt(bcch)))),
...
test_that("worldclim_extract() basic functionality with all expected data inputs.", {
  ...
  expect_silent(extracted <- suppressWarnings(suppressMessages(worldclim_extract(
    sf_pt,
  ...
  expect_named(extracted, c("SurveyAreaIdentifier", "latitude", "longitude", "survey_year", "survey_month", "survey_day", "geometry", "tavg"))
...
test_that("worldclim_extract() succeeds with alternate column names, either passed through attributes or specified explicitly.", {
  ...
  expect_silent(extracted <- suppressWarnings(suppressMessages(worldclim_extract(sf_pt,
                                                                                 worldclim_data = tavg_sf_pt))))
  expect_s3_class(extracted, "sf")
  expect_equal(as.character(sf::st_geometry_type(extracted, by_geometry = FALSE)), "POINT")
  expect_named(extracted, c("sites", "latitude", "longitude", "survey_year", "mth", "survey_day", "geometry", "tavg"))
  expect_true(inherits(extracted$tavg, "numeric"))
  expect_equal(dplyr::select(extracted, -"tavg"), sf_pt, ignore_attr = TRUE) # Ignores attributes to confirm that data has not been otherwise modified.
  expect_equal(format(sf::st_crs(extracted)), "Canada_Albers_Equal_Area_Conic")
  expect_equal(unname(apply(X = apply(FUN = is.na, X = extracted, MARGIN = 1), FUN = unique, MARGIN = 1)), rep(FALSE, times = 8))
