...
This script pre-compiles vignettes and replaces the figure paths
appropriately.  
Note that this *relies* on all figures having alt text and at least a
`fig.cap = ""` as this creates a different style of figure and the
replacement therefore works.

All precompiled vignettes also need to define the figure path to be in
the `vignettes/articles/figures` folder. Provide a suffix as well to
prevent duplicates.
...
![Two-panel plot visualizing the regional and masked
DTM's](figures/nc_test_unnamed-chunk-3-1.png)
...
> **Note:** Use the `\` at the end to ensure that the text is treated as
> *alt* text and not a caption
> ([ref](https://github.com/quarto-dev/quarto-cli/discussions/9310#discussioncomment-9056156))
