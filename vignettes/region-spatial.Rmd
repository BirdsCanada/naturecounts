...
```{r}
obs <- nc_data_dl(region = list(statprov = "MB"), verbose = FALSE)
```
...
```{r, echo = FALSE, eval = !file.exists(here("vignettes/region-spatial_files/ne_10m_urban_areas.shp"))}
# Run only if shape file not available
ne_download(scale = 10, type = "urban_areas", returnclass = "sf", 
            destdir = here("vignettes/region-spatial_files"), load = FALSE) 
```
...
```{r, echo = FALSE}
urban <- ne_load(scale = 10, type = "urban_areas", returnclass = "sf", 
                 destdir = here("vignettes/region-spatial_files"))
```
...
```{}
obs <- nc_data_dl(region = list(utm_squares = utm_on$utm_square), verbose = FALSE)
```
...
```{r, echo = FALSE, eval = !file.exists("vignettes/region-spatial_files/Parks_Protected_Areas_Alberta.shp")}
...
download.file(url = "https://www.albertaparks.ca/media/2941843/parks_and_protected_areas_alberta.zip",
              destfile = here("vignettes/region-spatial_files/alberta_parks.zip"))
unzip(here("vignettes/region-spatial_files/alberta_parks.zip"), 
      exdir = here("vignettes/region-spatial_files/"))
```
...
```{r echo = FALSE}
parks <- st_read(here("vignettes/region-spatial_files/Parks_Protected_Areas_Alberta.shp"))
```
...
```{r}
obs <- nc_data_dl(region = list(bbox = b), verbose = FALSE)
```
