...
You can only use **one** type of region, but you can filter to multiple regions of that type:
...
Here we'll go through in detail how to use the various codes. Note that in some
examples we also filter by species just so things don't take so long to download.
...
```{r}
nc_count(species = 133990, region = list(country = "US"))
```
