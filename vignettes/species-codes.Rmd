...
```{r}
moose <- nc_data_dl(collections = "all", species = 133990, verbose = FALSE)
```

Or by saving and referencing the data frame:
```{r}
moose_id <- species_search("moose")$species_id
moose <- nc_data_dl(collections = "all", species = moose_id, verbose = FALSE)
```
...
chickadees <- nc_data_dl(collections = "all", species = chickadee_ids, verbose = FALSE)
```
...
```{r}
mice <- nc_data_dl(species = mice_ids$species_id, verbose = FALSE)
mice
```
