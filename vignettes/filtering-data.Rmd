...
In this vignette we will go over several different types of filtering using the `tidyverse` collection of packages. In particular, we will make extensive use of the `filter()` function from the `dplyr` package. For more information about this function, see the chapater on [Data Transformations](https://r4ds.had.co.nz/transform.html#filter-rows-with-filter) in R for Data Science by Hadley Wickham and Garrett Grolemund.
...
con <- nc_data_dl(species = c(14270, 14280, 14290), sql_db = "chickadee", verbose = FALSE)
...
2. Filter the categores you a) wish to keep, or b) wish to exclude
