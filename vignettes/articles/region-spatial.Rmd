...
For convenience, we'll use the `grid.arrange()` function from the gridExtra to line up the figures side-by-side so we can get a really good look at what we're doing.
```{r}
grid.arrange(
  ggplot() +
    theme_bw() + 
...
So now we can see that the `utm_squares` we've selected overlap all our urban areas. Now we can download the observations for all of these areas. Because it can take time for the server to process so many `utm_squares`, we'll increase the timeout to 5 minutes.

```{r}
obs <- nc_data_dl(region = list(utm_squares = utm_on$utm_square), 
                  verbose = FALSE, username = "sample", timeout = 300)
```
