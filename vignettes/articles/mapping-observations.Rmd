...
years <- barred_owls_sf %>%
  group_by(years) %>%
  summarize(n = length(unique(survey_year)))
```
...
Here we download data from the `RCBIOTABASE` collection, make sure to keep only observations where all species and the location were reported, create a new `presence` column which is either TRUE, FALSE, or NA. Finally we use the `format_zero_fill()` function to fill in sampling events where cardinals (`species_id` 19360) were not detected (presence would then be 0).

```{r}
cardinals <- nc_data_dl(collection = "RCBIOTABASE", username = "sample") %>%
  filter(AllSpeciesReported == "Yes", !is.na(latitude), !is.na(longitude)) %>%
  mutate(presence = as.numeric(ObservationCount) > 0) %>%
...
```{r}
cardinals_sf <- st_as_sf(cardinals, coords = c("longitude", "latitude"))

map <- get_stamenmap(bbox = as.numeric(st_bbox(cardinals_sf)), zoom = 8)

ggmap(map, base_layer = ggplot(data = cardinals)) +
