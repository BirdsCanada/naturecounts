...
After downloading the WorldClim data to your working directory, list all the files in the raster stack by specifying the path to your folder (use `list.files` function) and read them into R using the `rast` function.
...
Convert the **species_occurrences_summary**, which summarizes species occurrence by SiteCode, into a spatial object using the `st_as_sf` function:
