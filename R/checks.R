username_check <- function(u) {
  if (missing(u)) {
    stop(
      "A 'username' is needed to access the server\n  ",
...
year_check <- function(y) {
  ...
  if (!is.numeric(y) || y > lubridate::year(Sys.Date()) || y < 1900) {
    stop(
      "Years must be numbers between 1900 and ",
...
bbox_check <- function(b, type) {
  if (type %in% c("bbox_left", "bbox_right")) {
    if (abs(b) > 180) {
      stop(
        "Bounding box longitude ('left'/'right' bound) ",
    ...
    if (abs(b) > 90) {
      stop(
        "Bounding box latitude ('top'/'bottom' bound) ",
...
iba_check <- function(i) {
  ...
  if (length(w) > 0) {
    stop(
      "Some arguments in 'iba' are invalid (",
      paste0(w, collapse = ","),
...
bcr_check <- function(b) {
  ...
  if (length(w) > 0) {
    stop(
      "Some arguments in 'bcr' are invalid (",
      paste0(w, collapse = ","),
  ...
  if (!is.null(project_ids)) {
    ...
    if (nrow(c1) == 0) {
      stop(
        ...
        "'project_id's specified in 'meta_collections()'",
        call. = FALSE
      )
    }
  ...
  if (!is.null(c) && (!is.character(c) || length(c1) > 0)) {
    ...
    if (any(c1 %in% meta_collections()$collection)) {
      msg <- paste0(
        ...
        paste0(c1[c1 %in% meta_collections()$collection], collapse = ", ")
      )
    }
    if (any(!c1 %in% meta_collections()$collection)) {
      msg <- paste0(
...
authority_check <- function(a) {
  if (!all(a %in% meta_species_authority()$authority)) {
    stop(
      "'authority' must be one or more of the authorities ",
      "specified in 'meta_species_authority()'.",
      call. = FALSE
    )
  }
}
...
fields_set_check <- function(fields_set) {
  ...
  if (length(fields_set) > 1 || !fields_set %in% c(v$version, "custom")) {
    if (!fields_set %in% v$shorthand) {
      stop(
        "'field_set' must be either a 'version' or a 'shorthand' code ",
        "returned by 'meta_bmde_version()', or 'custom'",
        call. = FALSE
      )
    }
...
fields_check <- function(fields) {
  ...
  if (length(w) > 0) {
    stop(
      "Some arguments in 'fields' are invalid (",
      paste0(w, collapse = ","),
      "). See 'meta_bmde_fields()' for valid options",
      call. = FALSE
    )
  }
  fields
...
codes_check_each <- function(desc, type, df) {
  ...
  if (
    ...
    stop(
      "'species' code must be a numeric code ",
      "(see documentation)",
      call. = FALSE
    )
  }
...
codes_convert <- function(desc, type) {
  c <- search_region(desc, type) %>%
    dplyr::select(dplyr::contains("_code"), dplyr::contains("_name")) %>%
    dplyr::distinct()
...
df_db_check <- function(df_db, collect = TRUE, verbose = TRUE) {
  if (!inherits(df_db, c("data.frame", "SQLiteConnection", "tbl_sql"))) {
    stop(
      "'df_db' must be either a data frame, a connection to an SQLite ",
      "database, or a connection to an SQLite table",
      call. = FALSE
    )
  }
  ...
  if (collect && inherits(df_db, c("SQLiteConnection", "tbl_sql"))) {
    ...
    if (inherits(df_db, "SQLiteConnection")) {
      if (!"naturecounts" %in% DBI::dbListTables(df_db)) {
        stop(
          "If 'df_db' is a SQLite database, it must have a 'naturecounts' ",
          "table",
          call. = FALSE
        )
      }
