...
filter_unpack <- function(f) {
  ...
  if (!is.null(f$years)) {
    names(f$years) <- c("start_year", "end_year")
  }
  if (!is.null(f$doy)) {
    names(f$doy) <- c("start_doy", "end_doy")
  }
...
filter_check <- function(f) {
  for (i in 1:length(f)) {
    n <- names(f[i])
    if (n == "fields_set") {
      f[[i]] <- fields_set_check(f[[i]])
    }
    if (n == "fields") {
      f[[i]] <- fields_check(f[[i]])
    }
    if (n == "utm_squares") {
      f[[i]] <- utm_check(f[[i]])
    }
    ...
    if (n == "site_type") {
      f[[i]] <- site_type_check(f[[i]])
    }
    # Check collections earlier, not here
    if (stringr::str_detect(n, "_year")) {
      f[[i]] <- year_check(f[[i]])
    }
    if (stringr::str_detect(n, "_doy")) {
      f[[i]] <- doy_check(f[[i]])
    }
    if (stringr::str_detect(n, "bbox")) {
      f[[i]] <- bbox_check(f[[i]], type = n)
    }
...
filter_redundancy <- function(f) {
  ...
  if (length(f[!names(f) %in% c("fields", "fields_set")]) > 3) {
    stop(
      "Only 3 arguments from `collections`, `species`, `years`, `doy`, ",
      "`region`, and `site_type` are permited. Remove ",
      length(f) - 3,
    ...
    if (length(region) > 1) {
      ...
      if (all(names(region) %in% c("country", "statprov", "subnational2"))) {
        locs <- factor(
          ...
          sort()
        message(
          "'country', 'statprov' and 'subnational2' are redundant, ",
          "keeping only '",
          locs[length(locs)],
          "'"
        )
        ...
        stop(
          "'region' can only be one of 'country', 'statprov', ",
