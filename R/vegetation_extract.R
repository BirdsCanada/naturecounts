...
vegetation_extract <- function(
  ...
  modis_files$productiondate <- yearyearday(
    as.numeric(substr(gsub(pattern = ".*modis", replacement = "./modis", modis_files$filename), 61 - 16, 61 - 13)),
    as.numeric(substr(gsub(pattern = ".*modis", replacement = "./modis", modis_files$filename), 61 - 12, 61 - 10))
  ) +
    lubridate::hms(paste0(
      substr(gsub(pattern = ".*modis", replacement = "./modis", modis_files$filename), 61 - 9, 61 - 8),
      ":",
      substr(gsub(pattern = ".*modis", replacement = "./modis", modis_files$filename), 61 - 7, 61 - 6),
      ":",
      substr(gsub(pattern = ".*modis", replacement = "./modis", modis_files$filename), 61 - 5, 61 - 4)
  ...
  for (i in `if`(
    ...
    for (j in unique(modis_match$filename)) {
      ...
      if(reliability == TRUE){
        modis_reliability <- terra::rast(j)["\"500m 16 days pixel reliability\""]
      }
      ...
      for (k in unique(pts_to_fill$SurveyAreaIdentifier)) {
        ...
        if (buffered == TRUE) {
          ...
          if(reliability == TRUE){
            pixel_vals <- table(exactextractr::exact_extract(modis_reliability_clip, tmp)[[1]]$value)
            
            str_components <- c()
            for(l in 1:length(pixel_vals)) {
              labels <- data.frame(value = c(-1:3),
                                   label = c("Fill/No Data",
                                             "Good Data",
                                             "Marginal Data",
                                             "Snow/Ice",
                                             "Cloudy"))
              str_components[l] <- paste0(labels$label[labels$value == names(pixel_vals)[l]], " (", round((unname(pixel_vals[l])/sum(pixel_vals))*100,2), "%)")
            }
          ...
          if(reliability == TRUE){
            ...
            data[
              ...
              "vegetation_reliability"
            ] <- labels$label[labels$value == unique(terra::extract(modis_reliability, tmp)[, 2])]
          }
        }
      }
    }
