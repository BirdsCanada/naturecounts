...
standardized format for use in the various covariate download and
extraction functions available in the naturecounts R package. Users are
not required to use this function before using covariate download and
extraction functions, but may avoid some finnicky work by doing so.
...
- data:
  ...
  'polygons' object containing observations associated with coordinate
  and date data.
...
- crs:
  ...
  Character. Optional argument to provide the coordinate reference
  ...
  [WGS84](https://epsg.io/4326) (`EPSG:4326`) coordinate reference
  system, or `sf`/ `terra` objects without coordinate reference systems
  embedded.

## Value

If `data.frame`, `sf` "POINT", or `terra` "points" data provided, `sf`
"POINT" object. If `sf` "POLYGON" or `terra` "polygons" data provided,
`sf` "POLYGON" object. Returned object contains a row for each unique
site-date combination in the provided data, and is provided in the [NAD
1983 Albers Canada](https://epsg.io/102001) (`EPSG:102001`) coordinate
reference system with the following columns.

- SurveyAreaIdentifier - character. Site names, or if missing in
  original data, filled site names for use in later functions.
...
to buffer data points by a specified distance to measure covariates at
