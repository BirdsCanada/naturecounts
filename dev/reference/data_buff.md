...
``` r
data_buff(data, buffer = TRUE, buffer_distance = 500, buffer_units = "m")
```
...
- buffer:
  ...
  Logical. Should the provided data be buffered? `TRUE` by default.

- buffer_distance:

  Numeric. Distance that the provided points or polygons should be
  buffered by. 500 by default.

- buffer_units:

  Character. Units of provided distance. Options are "m" (metres), "km"
  (kilometers), "ft" (feet), "yd" (yards), "mi" (miles), or "naut_mi"
  (nautical miles). Metres ("m") by default.

## Value

Either `sf` 'POLYGON' or `terra` 'polygons' (depending on format of
...
points are buffered to a circle with the radius specified in
`buffer_distance` (500 m by default). If input is an `sf` 'POLYGON'
object or a `terra` 'polygons' object, the polygon is buffered on all
sides by the distance specified in `buffer_distance` (500 m by default).

## See also

[`data_fmt()`](https://birdscanada.github.io/naturecounts/dev/reference/data_fmt.md)
for a convenient way to convert `data.frame` data to a compatible `sf`
or `terra` object.
...
#> [Data Buffering] buffering sites by 1km radius.
```
